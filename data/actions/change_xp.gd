...
func _get_txt() -> String:
	...
	match target:
		TARGET.FOCUSED:
			return Utils.translate("Selected Ponygirl %s %s XP") % [verb_s, amt]
		TARGET.RANDOM:
			return Utils.translate("A random active ponygirl %s %s XP") % [verb_s, amt]
		TARGET.ACTIVE:
			return Utils.translate("All active ponygirls %s %s XP") % [verb_p, amt]
		TARGET.ALL:
			return Utils.translate("All ponygirls %s %s XP") % [verb_p, amt]
	return ""
...
func get_result() -> String:
	...
	if target == TARGET.FOCUSED:
		text = Utils.translate("{PONYNAME} %s %s XP") % [verb_s, amt]
	if target == TARGET.RANDOM and not affected_ponygirls.is_empty():
		text = Utils.translate("%s %s %s XP") % [affected_ponygirls[0].name, verb_s, amt]
